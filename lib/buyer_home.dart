...
import 'package:trade_buyer/buyer_add_details.dart';
import 'package:trade_buyer/buyer_profile.dart';
import 'package:trade_buyer/buyer_login.dart';
import 'package:trade_buyer/buyer_item_widget.dart';
...
class _BuyerHomeState extends State<BuyerHome> {
  ...
  final List<Item> _buyersItems = []; // To hold buyer items from "sell_items"
  ...
  void initState() {
    super.initState();
    _fetchItems();
    _fetchBuyersItems(); // Fetch buyer items on init
  ...
  void _addItem() async {
    final newItem = await Navigator.push<Item>(
      context,
      MaterialPageRoute(
        builder: (context) => BuyerAddDetails(),
      ),
    );
  ...
  // Fetch buyers' items from the "sell_items" collection in Firebase
  Future<void> _fetchBuyersItems() async {
    try {
      final querySnapshot =
          await FirebaseFirestore.instance.collection('sell_items').get();

      final buyersItems = querySnapshot.docs
          .map(
      ...
      setState(() {
        _buyersItems.clear();
        _buyersItems.addAll(buyersItems);
      });

      print('Buyers items fetched successfully from Firebase!');
    } catch (error) {
      print('Failed to fetch buyers items: $error');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: !_isSearching
            ? Text(_selectedIndex == 0 ? "View Products" : "My Requirements")
            : TextField(
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              Navigator.pushReplacement(
                ...
                MaterialPageRoute(
                    builder: (context) =>
                        BuyerAuthScreen()), // Navigate back to AuthScreen
              );
            },
          ),
        ],
  ...
  // Builds the list for buyers' items
  Widget _buildBuyersList() {
    if (_buyersItems.isEmpty) {
      return const Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
    ...
    return ListView.builder(
      itemCount: _buyersItems.length,
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
      itemBuilder: (context, index) {
        final item = _buyersItems[index];
        final String formattedDate = formatDateTime(item.timestamp.toDate());
